...
inline std::string makeTimestampedFilename(const std::string &prefix = "run",
...
inline void openLogFile(const std::string &prefix = "log/run") {
  ...
  logPrintf(LogLevel::info, "Logging to %s",
            getLoggerConfig().filepath.c_str());
}
